...
class InfoFeature : BukkitRunnable() {
    ...
    private val announcements = listOf(
        ...
        Chat.colored("$prefix Like our server @ ${Chat.secondaryColor}namemc.com/server/${if (SettingsFeature.instance.data!!.getString("config.chat.serverIp") != null) SettingsFeature.instance.data!!.getString("config.chat.serverIp") else "no server ip setup in config tough tits"}&7!"),
        ...
        Chat.colored("$prefix Follow us on twitter &b@${if (SettingsFeature.instance.data!!.getString("config.chat.twitterHandle") != null) SettingsFeature.instance.data!!.getString("config.chat.twitterHandle") else "no twitter handle setup in config tough tits"}&7!"),
        Chat.colored("$prefix Get notified for games on twitter, follow us at &b@${if (SettingsFeature.instance.data!!.getString("config.chat.twitterHandle") != null) SettingsFeature.instance.data!!.getString("config.chat.twitterHandle") else "no twitter handle setup in config tough tits"}&7!"),
