        ...
        if (args.size == 0) {
            ...
            if (GameState.currentState == GameState.LOBBY) {
                if (sender.world.name == "Arena") {
                    ...
                    sender.sendMessage(Chat.colored("&8[&cArena&8]&7 &7You left the arena."))
                    return false
                }
                if (!JavaPlugin.getPlugin(Kraftwerk::class.java)!!.arena) {
                    sender.sendMessage(Chat.colored("&8[&cArena&8]&7 &7Arena is currently disabled."))
                    return false
                }
                ArenaFeature.instance.send(sender)
                Chat.sendMessage(sender, "&8[&cArena&8]&7 Welcome to the arena, &f${sender.name}&7!")
